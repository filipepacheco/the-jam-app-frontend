...
- Scene color: each song gets its own stage-light color. The color is a hue shift from the theme primary, so lightness and chroma follow the theme. A hash of the song id selects the shift from a fixed list, so all screens show the same color. The lights change only after the new song fully arrives, at the end of its roll or its flight. Then they blend to the new color over 2.1 seconds and take the short way round the color wheel. The applause gold comes at once. Text, the next card and the invitation card keep the brand color. Browsers without relative colors keep the brand colors.
...
- Lineup edit: only the new or renamed musician moves, and a highlight marks the group. A change on the song on stage is never announced: the lineup only changes with this transition.
- Join spotlight: a new sign-up for a queued song gets an announcement ("Diego entrou na jam!"). A join is a named musician who is new to the whole queue. A move between songs, a second sign-up, and a song that only slides into the list when the queue moves on do not count. A sign-up for the song on stage is not announced. The place of the announcement depends on the song:
  - The up-next song: the announcement covers the up-next card. After 3.3 seconds, each name flies to its slot in the up-next lineup, and the group highlight flashes when it lands. The slot stays hidden until then, but keeps its space.
  - A later song: the announcement rises over the steps under the QR code, and then sinks back. The QR code does not move.
  One poll of joins makes one announcement for each place, with a maximum of three names, and the rest show as a count. The two places can show at the same time. Both wait during the applause and the up-next flight, and an applause that starts clears them at once. If the up-next song goes to the stage before its announcement plays, the announcement is dropped. Every announcement ends after a maximum of 10 seconds, so a name never stays hidden.
...
- Applause: when a song that the room heard leaves the stage, the stage thanks its band for 10 seconds ("Palmas para Yuri, Alexandra e Marina!"). A song was heard if it was playing or paused while on stage. The title names the band, so the lineup fades out, but it keeps its space and nothing moves. The lights turn gold and open wide, and the change flash pulses three times. The lineup shows again when the next song arrives. Square confetti in the theme colors falls over the whole display for the full applause, and the last pieces then fall off. The next card keeps the incoming song during the hold. Then both cards move to the latest data together. If the host goes back to the same song, the applause stops. `useStageHandover` makes these decisions during render, so the stage never shows the next song first. The carousel layout does not use it.
...
- Audience reactions: while the Jam is `LIVE`, the public Jam page shows a reaction bar with four buttons: clap, fire, heart and rock. The phone merges the taps of 400 ms into one message for each kind, with a maximum count of 10. The message goes on a Supabase Realtime broadcast channel for the Jam, and nothing is stored. The venue display drops any message that is not a known kind with a whole count from 1 to 10. Each reaction rises as an emoji behind the stage text, and a batch spreads over 600 ms. A maximum of 30 emoji show at one time, so a flood cannot hide the stage. During the applause, the label shows a live clap count ("30 palmas").
- Named reactions: a signed-in person sends the account name with each message. Nobody types text. The venue display shows who reacted in a feed at the bottom-right corner of the stage card, for example "Lipe amou isso!" with a heart emoji. The name has no quotes. A guest sends no name, and the line names "A Plateia" ("The Crowd"), for example "A Plateia amou isso!". All guests share one line for each kind. The reaction bar tells the person what the screen shows. A signed-in person sees "Seu nome aparece no telão". A guest sees "Entre para mostrar seu nome no telão".
- Feed lines: the feed shows a maximum of 3 lines. A new line rises in at the bottom, and the other lines move up. A line stays for 4 s after its last reaction, and then it fades out. A new reaction with the same name and kind refreshes its line. It does not add a line. When the feed has 3 lines, a new line removes the line with the oldest last reaction. In reduced-motion mode, the lines use opacity only. They do not rise or move.
- Reaction security: the channel is public. Any sender with the anon key can send to it, and any sender can claim any name. The display does not trust the name. It removes control and format characters (bidi overrides included), changes each run of whitespace to one space, and shows a maximum of 32 characters. The display shows the name as text, never as HTML. The upgrade path is a private channel behind a backend relay. The relay adds the verified account name, and the display code does not change.
...
- Card heights: a song change can make a card taller or shorter. Then the rows ease to their new sizes over 1.35 seconds, on the curve of the up-next flight, so the flight lands on the final layout. The text blocks in each card glide from their old places, so nothing jumps. While the rows move, a card clips the content that does not fit yet. At the finale, the next card fades out in its old place while the stage grows into the space. A poll that changes nothing does not restart a resize. `VenueProgramme` measures the layout with `getSnapshotBeforeUpdate`, before React changes the DOM.
...
Reduced-motion mode stops all movement and ambient motion. Song and lineup changes then use a short opacity fade: 120 ms out and 200 ms in. The banner, the controls panel and the carousel also fade without movement. The applause still shows, but without confetti. The join spotlight crossfades in and out, and names do not fly. The up-next song does not fly; the cards use the opacity fade. The cards take their new heights at once. Reactions glow in place with no travel, and a maximum of 8 show at one time. The named feed lines fade in and out with no movement.
Hidden documents stop all cues.
Rapid updates cancel the previous cue, and QR content never moves.
Classic mode has no confetti on each song change. Confetti falls only during the applause.
